...
for(let i= 0; i<pencilColors.length; i++){
    pencilColors[i].addEventListener("click", function(e){
        console.log(pencilColors[i])
        let selectedPencilColor = pencilColors[i].getAttribute("class");
        console.log(selectedPencilColor);
...
pencil.addEventListener("click" , function(){
    if(activeTool == "pencil"){
        ...
        eraserOptions.classList.add("hide");    //why
...
eraser.addEventListener("click" , function(){
    if(activeTool == "eraser"){
        ...
        pencilOptions.classList.add("hide");    //why
    }
})